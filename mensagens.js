class MessagesSystem {
        ...
        container.innerHTML = Object.keys(groupedMessages).map(date => `
            <div class="message-group">
                ...
                ${groupedMessages[date].map(msg => {
                    // ⭐⭐ DETECTAR E RENDERIZAR STICKERS
                    if (msg.message && msg.message.includes('[STICKER]')) {
                        try {
                            const stickerMatch = msg.message.match(/\[STICKER\](.*?)\[\/STICKER\]/);
                            if (stickerMatch) {
                                const stickerData = JSON.parse(stickerMatch[1]);
                            ...
                            console.error('Erro ao renderizar sticker:', e);
                        }
                    }
    ...
    async getUserStatus(userId) {
        ...
        try {
            ...
            // ⭐⭐ VERIFICAR SE É STICKER PARA CONTAGEM ESPECÍFICA
            const isSticker = message.includes('[STICKER]');
            
            if (!isPremium) {
                const canSend = await this.checkCanSendMessage(isSticker);
                if (!canSend.can_send) {
            ...
            if (data === 'success') {
                // ⭐⭐ ATUALIZAR CONTADOR APÓS ENVIO BEM-SUCEDIDO
                if (!isPremium) {
                ...
                this.showMessageStatus(isSticker ? 'Sticker enviado!' : 'Mensagem enviada!', 'success');
            ...
            this.showMessageStatus('Erro ao enviar', 'error');
    ...
    // ⭐⭐ NOVA FUNÇÃO: INCREMENTAR CONTADOR DE MENSAGENS/STICKERS
    async incrementMessageCounter(isSticker = false) {
        try {
            ...
            const { data: limits, error } = await this.supabase
                ...
                .select('messages_sent_today, stickers_sent_today')
            ...
            if (error) {
                ...
                await this.supabase
                    ...
                    .upsert({
                        ...
                        messages_sent_today: isSticker ? 0 : 1,
                        stickers_sent_today: isSticker ? 1 : 0,
    ...
    async sendMessageFallback(message) {
        try {
            ...
            if (data) {
                // ⭐⭐ ATUALIZAR CONTADOR NO FALLBACK TAMBÉM
                let isPremium = false;
                if (this.currentUser.profile?.is_premium) {
                    isPremium = this.currentUser.profile.is_premium;
                }
    ...
    async resetDailyCounterIfNeeded() {
        try {
            const { data: limits, error } = await this.supabase
                .from('user_message_limits')
                .select('messages_sent_today, stickers_sent_today, last_reset_date')
                .eq('user_id', this.currentUser.id)
                .single();
    ...
    async checkCanSendMessage(isSticker = false) {
        try {
            await this.resetDailyCounterIfNeeded();
            ...
            // ⭐⭐ LÓGICA SEPARADA PARA STICKERS
            if (isSticker) {
                const stickersSentToday = limits.stickers_sent_today || 0;
                if (stickersSentToday >= this.stickerLimit) {
                    return { can_send: false, reason: 'sticker_limit_reached' };
                }
            } else {
                // Lógica normal para mensagens de texto
                const messagesSentToday = limits.messages_sent_today || 0;
                if (messagesSentToday >= this.messageLimit) {
    ...
    async updateMessageCounter() {
        try {
            ...
            counter.innerHTML = `
                <span class="counter-text">Mensagens: ${sentToday}/${this.messageLimit} | Stickers: ${stickersSentToday}/${this.stickerLimit}</span>
            `;
    ...
    setupEventListeners() {
        ...
        if (stickersBtn) {
            stickersBtn.addEventListener('click', () => {
                ...
                } else if (window.openStickersModal) {
                    window.openStickersModal();
                }
            });
        }
    }
